...
pytesseract.pytesseract.tesseract_cmd = r'C:/Users/Administrator/Desktop/Files/Tesseract-OCR/tesseract.exe'
...
def recommend_jobs(generated_content, jobs):
    ...
    Identify Key Skills and Experiences: Analyze the resume to extract the most relevant skills, technical proficiencies, and professional experiences.
    Match to Job Requirements: Compare these extracted skills and experiences to the job descriptions in the provided list.
    Prioritize Recommendations: Rank the matched jobs based on the degree of overlap between the candidate's qualifications and the job requirements.
    Output Format:
    Template :
    ...
    Dont give the less relevant matches..
    **If no job given then Tell only "No Jobs were found..
    """
...
def fetch_job_details_from_db(job_dict):
    """Fetch job details from the database based on job titles and links in the provided dictionary."""
    connection = None
    job_details = {}
    ...
    try:
        ...
        for job_title, job_link in job_dict.items():
            # SQL query to fetch job details matching the title and link
            cursor.execute("""
                SELECT title, company, link, experience, salary, location, description, image_link, date_of_post 
                FROM core_job 
            ...
            if result:
                # Create a dictionary for the job details
                job_details[job_title] = {
                    ...
                    'image_link': result[7],
                    'date_of_post': result[8],
                }
                # Print the job details
                print(f"Job Title: {result[0]}")
                print(f"Company: {result[1]}")
                print(f"Link: {result[2]}")
                print(f"Experience: {result[3]}")
                print(f"Salary: {result[4]}")
                print(f"Location: {result[5]}")
                print(f"Description: {result[6]}")
                print(f"Image Link: {result[7]}")
                print(f"Date of Post: {result[8]}\n")

    except mysql.connector.Error as error:
        print(f"Error fetching job details from database: {error}")
    finally:
...
def main_offer(request):
    ...
    pdf_path = os.path.join(r'C:\Users\Administrator\3D Objects\rit_placement\media\resume', f'{student_id}.pdf')
...
# Initialize an empty dictionary for job recommendations
        ...
        # # Format recommendations in markdown style
        # markdown_recommendations = "## Job Recommendations\n\n" + "\n".join(f"* **{job}**" for job in recommendations.splitlines() if job) 

        # # Return the recommendations in a markdown response
        return HttpResponse(job_dict, content_type='text/plain')

    return HttpResponse("No images found.", status=404)  # Return a response if no images were processed
